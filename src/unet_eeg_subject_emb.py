...
class SelfAttention(nn.Module):
    def __init__(self, embed_size):
        ...
        self.query = nn.Conv2d(embed_size, embed_size, 1)  # 1x1 conv
        self.key = nn.Conv2d(embed_size, embed_size, 1)
        self.value = nn.Conv2d(embed_size, embed_size, 1)
...
class sub_gaussion(Module):
    ...
    def __init__(self, eeg_channels: int = 3, n_channels: int = 22,
        ...
        * `n_blocks` is the number of `UpDownBlocks` at each resolution
        ...
        self.final_mu = nn.Conv2d(n_channels, 62, kernel_size=(3, 3), padding=(1, 1))
        self.final_theta = nn.Conv2d(n_channels, eeg_channels, kernel_size=(3, 3), padding=(1, 1))
        #Attention
        self.attention_mu = SelfAttention(n_channels)
        self.attention_theta = SelfAttention(n_channels)
        #channel
        self.prochannel_seedv = nn.Conv2d(eeg_channels, out_channels=62, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, s: torch.Tensor, debug=False):
        ...
        * `t` has shape `[batch_size]`
        ...
        #self-attention
        # mu = self.attention_mu(mu)
        theta = self.attention_theta(theta)
        # theta = self.final_theta(self.act(self.norm(theta)))
        # mu = self.final_mu(self.act(self.norm(theta)))
        # Final normalization and convolution
        return self.final_mu(self.act(self.norm(theta))), self.final_theta(self.act(self.norm(theta)))
