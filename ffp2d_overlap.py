...
class DenoiseDiffusion:
    ...
    def p_sample(self, xt: torch.Tensor, t: torch.Tensor):
        ...
        model = self.eps_model(xt, t)
        # ================================
        # model = self.eps_model(64, self.out_channels, [30] * 4, [6] * 4)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = model.to(device)
        a = xt
        self.image_proj = nn.Conv2d(22, 64, kernel_size=(3, 3), padding=(1, 1)).to(device)
        xt = self.image_proj(xt)
        xt = xt.to(device)
        eps_theta = model(xt)
        xt = a
        with torch.no_grad():  # 禁用梯度计算，适用于推理阶段
    ...
    def loss_with_diff_constraint(self, x0: torch.Tensor, label: torch.Tensor,
        ...
        # 测量显存使用
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = self.eps_model.to(device)
        ...
        # model = self.eps_model(64,self.out_channels,[30] * 4, [6] * 4)
        # eps_theta = model(xt)
        # total_params = sum(p.numel() for p in self.eps_model.parameters() if p.requires_grad)
        # print(f"Total trainable parameters: {total_params}")


        # output = self.eps_model(xt, t)  # 执行模型推理
        ...
        # 计算 FLOPS 和参数
        flops, params = profile(self.eps_model, inputs=(xt,t))
