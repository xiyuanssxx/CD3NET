...
from src.EEGNET3 import EEG_Net_8_Stack
...
from ffp2d_seedV_overlap_contrastive2 import Configs
...
class ArcMarginHead(nn.Module):
    ...
    def __init__(self, in_features, out_features,
                 load_backbone='/HOME/scz5346/run/xiaoye/DS-DDPM-main/assets/SEED-IV/max_acc.pth', s=30.0, m=0.50,
                 easy_margin=False):
...
class DenoiseDiffusion:
    ...
    def __init__(self, eps_model: nn.Module, n_steps: int, device: torch.device, sub_theta: nn.Module,
                 sub_arc_head: nn.Module, debug=False):
        """
    ...
    def loss_with_diff_constraint(self, x0: torch.Tensor, label: torch.Tensor,
        ...
        \bigg\Vert^2 \Bigg]$$
        """
        # print('loss_with_diff_constraint===')
        # Get batch size
        debug = self.debug
        batch_size = x0.shape[0]
        # Get random $t$ for each sample in the batch
        ...
        # return F.mse_loss(noise, eps_theta ) , constraint_panelty, noise_content_kl_co * noise_conent_kl
...
class Sampler:
    ...
    def __init__(self, diffusion: DenoiseDiffusion, eeg_channels: int, window_size: int, step: int, stack_size: int,
                 device: torch.device):
        """
        ...
        * `device` is the device of the model
        ...
        self.prochannel_seedv = nn.Conv2d(62, out_channels=22, kernel_size=1).to(device)
    ...
    def show_image(self, img, title=""):
        ...
        plt.title(title)
        plt.show()
        data_path = '/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults/visualize_celebA/{}.png'.format(title)
    ...
    def sample_animation(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
                         subject_id=[5], dataroot=None, apply_step=20):
        # print('sample_animation============')
        """
        ...
        for t_inv in monit.iterate('Denoise', apply_step):
            ...
            if t_ % interval == 0:
                ...
                # print('real_waves.shape',real_waves.shape)
                real_noises = np.ones((x0.shape[0], 22, 750))
                # print('real_noises.shape',real_noises.shape)
                if overlap_cover:
                ...
                dataroot = '/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults'  # ===
                ...
                plt.clf()
                # 所有数据，前六个通道，400个时间点（不确定））
                annimate_curve_change(test_X=real_waves[0:, 0:6, :750], given_path=curve_temp_path, color='b')
                annimate_curve_change(test_X=real_noises[0:, 0:6, :750], given_path=noise_temp_path, color='g')
            ...
            # print('t_noise.shape',xt_noise.shape)#===
            # 生成的扰动信号+生成的噪声信号（二选一，第一个是xt变化+noise归一，第二个先求和后归一
        ...
        # =================有问题
        # visualize_animate_surface(test_X=real_waves_list[-1], test_y=sample_labels, given_path='/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults')#===

        # Make video
        if create_video:
            self.make_video(frames)

    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
                         subject_id=1, batch_size=16, mix_batch_size = 9, mix_trail = 600):
        """
            sample EEG singals, Subject noise and clean contnet and save it to matlab format.
        """
        # print('sample_save_data============')
        # $x_T \sim p(x_T) = \mathcal{N}(x_T; \mathbf{0}, \mathbf{I})$
        xt_tmp = torch.randn([batch_size, self.eeg_channels, self.window_size, self.stack_size], device=self.device)
        # print('shape of xt is {}'.format(xt.shape))#===
        s_tmp = torch.tensor([subject_id] * batch_size, dtype=torch.long, device=self.device)
        # Interval to log $\hat{x}_0$
        interval = self.n_steps // n_frames
        # Frames for video
        frames = []
        ...
        real_waves_all = []
        for i in range(16):
            # print('xt_tmp',xt_tmp.shape)
            xt = xt_tmp[start_value : end_value,: , :, :]
            # print('xt',xt.shape)
            s = s_tmp[start_value : end_value]
            ...
            for t_inv in monit.iterate('Denoise', self.n_steps):
                ...
                # xt1 = self.prochannel_seedv(xt)
                # print('\\xt-after', xt.shape)
                ...
                if debug:
                    ...
                    print(subnoise_theta.shape)
                    print(eps_theta.shape)
                if t_ % interval == 0:#只有进入到这个if里才会影响xt的维度变化。
                    # Get $\hat{x}_0$ and add to frames
                    x0 = self.p_x0(xt, t, eps_theta)
                    x0_noise = self.p_x0(xt, t, subnoise_theta)
                    ...
                    xt_noise = F.normalize(xt_noise, p=2, dim=2)  # ===
                    xt = F.normalize(self.p_sample(xt, t, eps_theta) + xt_noise, p=2, dim=2)  # ===
                    xt = self.p_sample(xt, t, eps_theta)#根据上面那个print，以及查看了一下BCI的代码，发现到了49,BCI的代码也会进入到这个if，所以这个代码没问题。而且22是BCI的通道，那就需要修改下面这个引起xt维度变化的代码了。
                    # print('===============')
                    # print('x0_noise.shape',x0_noise.shape)
                    # print('xt.shape',xt.shape)
            ...
            real_noises_all.append(real_noises)
            real_mix_all.append(real_mix)
            real_waves_all.append(real_waves)
            start_value += mix_trail
            end_value += mix_trail

        real_mix = np.concatenate(real_mix_all, axis = 0)
        real_waves = np.concatenate(real_waves_all, axis = 0)
        real_noises = np.concatenate(real_noises_all, axis = 0)


        data_sampled_path = '/media/administrator/F62C65672C652439/yexiyuan/sample_data/seed-v/'
        output_path = os.path.join(data_sampled_path, 'motor4')
...
def visualize_animate_surface(test_X=None, test_y=None, given_path=None):
    # print('visualize_animate_surface==========')
    # if test_X is None:
    ...
    plt.savefig('/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults/visualization/montage.png')
    fig = plt.figure()
    plt.scatter(xp_1020, yp_1020, c='r', label='10-20system')
    plt.scatter(xp_64, yp_64, c='g', label='64-system')
    plt.savefig('/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults/visualization/montage_2d.png')
    # print(test_X.shape)
...
def annimate_curve_change_with_diff(test_X=None, test_y=None, given_path=None):
    # print('annimate_curve_change_with_diff===')
    plt.cla()
    if test_X is None:
...
def main():
    ...
    run_uuid = "cc1d1f5a349011ef8e28fda65057bf42"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
    # Load custom configuration of the training runs
    ...
    sampler = Sampler(diffusion=configs.diffusion,
                      ...
                      device=configs.device, window_size=configs.window_size, step=step_size,
    ...
    with experiment.start():
        ...
        with torch.no_grad():
            ...
            for i in range(1):
                # sampler.sample_animation(n_frames=1000, subject_id = [i], dataroot='/HOME/scz5346/run/xiaoye/DS-DDPM-main/myresults/')
                # sampler.sample_save_data(n_frames=20, subject_id=i, batch_size=600)
                # sampler.sample_save_data(n_frames=20, subject_id=i, batch_size=72)
                sampler.sample_save_data(n_frames=20, subject_id=i, batch_size=500 * 16, mix_trail=500)
