...
from src.EEGNET2 import EEG_Net_8_Stack
...
class DenoiseDiffusion:
    ...
    def loss_with_diff_constraint(self, x0: torch.Tensor, label: torch.Tensor,
        ...
        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # self.prochannel_seedv = nn.Conv2d(62, out_channels=22, kernel_size=1).to(device)
        # xt_=self.prochannel_seedv(xt)
        if debug:
            print("the shape of xt")
            print(xt.shape)
        # Get $\textcolor{lightgreen}{\epsilon_\theta}(\sqrt{\bar\alpha_t} x_0 + \sqrt{1-\bar\alpha_t}\epsilon, t)$
        # eps_theta = self.eps_model(xt, t)
        # model = FFPModel(3, 3, [30] * 4, [6] * 4)
        # eps_theta = self.eps_model(22, 22, [30] * 4, [6] * 4)
...
#=============================================
        ...
        if debug:
            ...
            print(loss_orth)
        subject_arc_logit = self.sub_arc_head(subject_mu.permute(0, 3, 2, 1), s)
...
class Configs(BaseConfigs):
    ...
    def init(self):
        ...
        self.sub_archead = ArcMarginHead(
            ...
            load_backbone='./assets/attention-SEEDV/max_acc.pth',
    ...
    def train(self):
        ...
        for data, label,*_ in monit.iterate('Train', self.data_loader):
        ...
        #         [loss.detach().cpu().numpy(), (loss - loss_orth - sub_arc_loss).detach().cpu().numpy(),
...
class DatasetLoader_BCI_IV_signle(torch.utils.data.Dataset):
    ...
    def __init__(self, setname, datafolder=None, train_aug=False, subject_id=1):
        ...
        data = sio.loadmat(data_folder + "/single_subject_data_" + str(subject_id) + ".mat")
...
#     """
#
#     return DatasetLoader_BCI_IV_signle('train', datafolder='./data/bci-2a',subject_id=1)
...
def datasetLoader_BCI_IV_mix_subjects(c: Configs):
    ...
    Create BCI IV dataset
    """
    return DatasetLoader_BCI_IV_mix_subjects('train', datafolder='./dataset/seedv/motor0')
