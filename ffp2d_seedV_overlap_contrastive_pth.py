...
from src.unet_cell import FFPModel
...
os.environ["WANDB_API_KEY"] = '3dec153fea4ba1771ed107444d2d476e0ddd80e0'
...
class ArcMarginHead(nn.Module):
    ...
    def __init__(self, in_features, out_features, load_backbone='./assets/max_acc.pth', s=30.0, m=0.50,
                 easy_margin=False):
        ...
        print(
            "loading the pretrained subject EEGNet weight and convert to arc..{}".format(pretrained_checkpoint.keys()))
...
class DenoiseDiffusion:
    ...
    def __init__(self, eps_model: nn.Module, n_steps: int, device: torch.device, sub_theta: nn.Module,
                 sub_arc_head: nn.Module, debug=False, time_diff_constraint=True):
        """
        ...
        * `device` is the device to place constants on
        ...
        self.subject_noise_range = 16
        # self.arcmargin = ArcMarginProduct()
        # self.out_channels =62
    ...
    def p_sample(self, xt: torch.Tensor, t: torch.Tensor):
        ...
        #================================
        model = self.eps_model(22, self.out_channels, [32] * 4, [6] * 4)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = model.to(device)
        a = xt
        # self.image_proj = nn.Conv2d(62, 64, kernel_size=(3, 3), padding=(1, 1)).to(device)
        # xt = self.image_proj(xt)
        xt = xt.to(device)
    ...
    def loss_with_diff_constraint(self, x0: torch.Tensor, label: torch.Tensor,
                                  ...
                                  eeg_channels: int = 22, n_channels: int = 64):
        """
        #### Simplified Loss
...
#===========================================
        ...
        # self.image_proj = nn.Conv2d(eeg_channels, n_channels, kernel_size=(3, 3), padding=(1, 1)).to(device)
        # xt = self.image_proj(xt)
        xt = xt.to(device)
        eps_theta = model(xt,t)
        ...
        # output = self.eps_model(xt, t)  #
        end_memory = torch.cuda.memory_allocated()

        #
        # memory_usage = (end_memory - start_memory) / (1024 ** 2)  # 转换为 MB
        # print(f"Memory Usage: {memory_usage:.2f} MB")
        # from thop import profile

        # FLOPS
        # flops, params = profile(model, inputs=(xt,t))
...
#=============================================
        ...
        total_params = sum(p.numel() for p in self.sub_theta.parameters() if p.requires_grad)
        # print(f"Total trainable parameters for subject: {total_params}")
        ...
        for i in range(eps_theta.shape[3] - 1):
            ...
            if debug:
                ...
                print(
                    F.mse_loss(eps_layernorm[:, :, (i + 1) * self.step_size: i * self.step_size + self.window_size, i],
                               eps_layernorm[:, :, (i + 1) * self.step_size: i * self.step_size + self.window_size,
                               i + 1], reduction='none').shape)
                print(F.mse_loss(eps_layernorm[:, :, self.step_size:, i], eps_layernorm[:, :, :-self.step_size, i + 1],
                                 reduction='none').shape)
                print(F.mse_loss(eps_layernorm[:, :, self.step_size:, i], eps_layernorm[:, :, :-self.step_size, i + 1],
                                 reduction='mean'))
                print(F.mse_loss(eps_theta[:, :, self.step_size:, i], eps_theta[:, :, :-self.step_size, i + 1],
                                 reduction='mean'))
            constraint_panelty = constraint_panelty + F.mse_loss(eps_layernorm[:, :, self.step_size:, i],
                                                                 eps_layernorm[:, :, :-self.step_size, i + 1],
                                                                 reduction='mean')
        ...
        def contrastive_loss(eps_theta, subject_theta):
            # 计算eps_theta和subject_theta之间的余弦相似度损失
            cosine_similarity = F.cosine_similarity(eps_theta, subject_theta, dim=-1)
            return 1 - cosine_similarity.mean()  # 余弦相似度损失

        # 计算对比损失
        contrast_loss = contrastive_loss(eps_theta, subject_theta)

        # 将对比损失加到loss_orth上
        loss_orth = loss_orth + contrast_loss
        if debug:
        ...
        #==
        _,predicted=torch.max(subject_arc_logit,dim=1)
        arc=(predicted==s).float().sum()
        total=s.size(0)
        acc=arc/total
        ...
        if self.time_diff_constraint:
            return F.mse_loss(noise,
                              eps_theta + subject_theta) + orgth_co * loss_orth + arc_subject_co * subject_arc_loss + 0.1 * constraint_panelty, constraint_panelty, noise_content_kl_co * noise_conent_kl, arc_subject_co * subject_arc_loss, orgth_co * loss_orth,contrast_loss
        else:
            return F.mse_loss(noise,
                              eps_theta + subject_theta) + orgth_co * loss_orth + arc_subject_co * subject_arc_loss, constraint_panelty, noise_content_kl_co * noise_conent_kl, arc_subject_co * subject_arc_loss, orgth_co * loss_orth,contrast_loss
...
class Configs(BaseConfigs):
    ...
    # is_attention: List[int] = [True, True, True, True]
    ...
    #FFPcell
    cells_out_channels=[32] * 4
    cells_exp= [6] * 4
    global_losses: list = []  # 明确声明属性
    fig, ax = plt.subplots(figsize=(10, 6))

    def plot_loss(self):
        """动态更新损失曲线"""
        self.ax.clear()
        self.ax.set_title('Real-time Loss Curve')
        self.ax.set_xlabel('Steps')
        self.ax.set_ylabel('Loss')
        self.ax.plot(self.global_losses, label='Train Loss')
        self.ax.legend()
        plt.pause(0.01)  # 短暂暂停用于更新图表
    ...
    def init(self):
        ...
        self.eps_model= FFPModel(
            ...
            cells_out_channels=self.cells_out_channels,
            cells_exp=self.cells_exp,
        ).to(self.device)
        # self.eps_model = Unet(
        ...
        self.sub_archead = ArcMarginHead(
            ...
            load_backbone='./assets/attention-MI/max_acc.pth',
        ...
        # Track and plot the loss
        self.global_losses = []  # 用于记录整个训练过程的所有损失
        self.fig, self.ax = plt.subplots(figsize=(10, 6))  # 初始化图表
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            for t_ in monit.iterate('Sample', self.n_steps):
                ...
                # print(f"Step {t_}: Time taken = {step_time:.6f} seconds")

            # print(f"Total sampling time: {total_time:.6f} seconds")
    ...
    def train(self):
        ...
        ### Train
        """
        # metric_file = '/home/lichunyu/Downloads/xiaohei/DS-DDPM-main/none_constraint_subject_gaussion_unet_att_loss_ep100_withorga_arc_normalized.csv'
        # loss_target = open(metric_file, 'a+')
        # target_writer = csv.writer(loss_target)
        # target_writer.writerow(['loss', 'time_period_diff'])  # ===
        from visdom import Visdom
        # 初始化一个空的损失列表，用于存储每个训练步骤的损失
        ...
        for data, label in monit.iterate('Train', self.data_loader):
            ...
            loss, time_period_diff, noise_conent_kl, sub_arc_loss, loss_orth, contrast_loss = self.diffusion.loss_with_diff_constraint(
                data, label)
            ...
            # Track and plot the loss
            self.global_losses.append(loss.item())  # Append the loss to the global list
            self.plot_loss()  # Update the plot dynamically

            # Track the loss
            tracker.save('loss', loss)
            # tracker.save('loss', loss)
            # tracker.save('diffusion_loss', loss - loss_orth - sub_arc_loss)
            tracker.save('mse', time_period_diff)
        #     tracker.save('noise_conent_kl', noise_conent_kl)
            tracker.save('sub_arc_loss', sub_arc_loss+loss_orth)
            tracker.save('contrast_loss', contrast_loss)
            # target_writer.writerow([
            #     loss.detach().cpu().numpy(),  # 当前损失
            #         (loss - loss_orth - sub_arc_loss).detach().cpu().numpy(),  # 去掉一些损失项后的损失
            #         time_period_diff.detach().cpu().numpy(),  # 时间段差异
            #         noise_conent_kl.detach().cpu().numpy(),  # 噪声内容的 KL 散度
            #         sub_arc_loss.detach().cpu().numpy(),  # 子模型损失
            #         loss_orth.detach().cpu().numpy()  # 正则化损失
            #     ])
            #
            # # 关闭 CSV 文件
            # loss_target.close()
...
class DatasetLoader_BCI_IV_mix(torch.utils.data.Dataset):
    ...
    def __init__(self, setname, datafolder=None, train_aug=False):
        ...
        if setname == 'train':
            ...
            self.data = real_test_win_x
            self.label = real_test_win_y
        scipy.io.savemat('train_label_shuffled.mat', {'train_label_shuffled': train_win_y})
        self.num_class = 4
    ...
    def __init__(self, setname, datafolder, train_aug=False):
        ...
        for i in range(9):
            ...
            if datafolder is None:
                ...
                data_folder = datafolder
# '/home/lichunyu/Downloads/xiaohei/DS-DDPM-main/data/single_sep (1)/single_sep(1)1-T'
            data = sio.loadmat(data_folder + "/single_subject_data_" + str(subject_id+1) + ".mat")
            # test_X = data["test_x"][:, :, 750:1500]  # [trials, channels, time length]
            train_X = data["train_x"][:, :, 750:1500]
...
#     """
#
#     return DatasetLoader_BCI_IV_signle('train', datafolder='/home/lichunyu/Downloads/xiaohei/DS-DDPM-main/data',
#                                        subject_id=3)
...
def datasetLoader_BCI_IV_mix_subjects(c: Configs):
    ...
    Create BCI IV dataset
    """
    return DatasetLoader_BCI_IV_mix_subjects('train', datafolder='./dataset/bci2a/motor4')
...
def main():
    ...
    experiment.create(name='none_constraint_subject_gaussion_unet_att_loss_ep100_withorga_arc',
                      writers={'screen', 'comet'})
