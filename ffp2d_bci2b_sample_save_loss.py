...
from ffp2d_bci2b_overlap_loss import Configs
    ...
    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
        ...
        if overlap_cover:
            ...
            for i in range(x0.shape[3]):
                ...
                real_noises[:, :, 0 + i * self.step: self.window_size + i * self.step] = x0_noise[:, :, :, i]
                real_mix[:, :, 0 + i * self.step: self.window_size + i * self.step] = x_mix[:, :, :, i]
        data_sampled_path = '/media/administrator/F62C65672C652439/yexiyuan/bci_2b_progan_loss'
...
def main():
    ...
    run_uuid = "e17b38c4efcf11ee8daa7b9204547487"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
