...
class ArcMarginHead(nn.Module):
    ...
    def __init__(self, in_features, out_features,
                 load_backbone='./assets/SEED-IV/max_acc.pth', s=30.0, m=0.50,
                 easy_margin=False):
    ...
    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
        ...
        if overlap_cover:
            ...
            for i in range(x0.shape[3]):
                ...
                real_noises[:, :, 0 + i * self.step: self.window_size + i * self.step] = x0_noise[:, :, :, i]
                real_mix[:, :, 0 + i * self.step: self.window_size + i * self.step] = x_mix[:, :, :, i]
        data_sampled_path = '/media/administrator/F62C65672C652439/yexiyuan/progan-contrastive-seediv'
        output_path = os.path.join(data_sampled_path, 'motor0')
...
def main():
    ...
    run_uuid = "e9538870fdef11ee8daa7b9204547487"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
