...
from ffp2d_seedV_overlap2 import Configs
    ...
    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
        ...
        if overlap_cover:
            ...
            for i in range(x0.shape[3]):
                ...
                real_noises[:, :, 0 + i * self.step: self.window_size + i * self.step] = x0_noise[:, :, :, i]
                real_mix[:, :, 0 + i * self.step: self.window_size + i * self.step] = x_mix[:, :, :, i]
        data_sampled_path = 'ffp2d_seedIV_overlap2/'
        output_path = os.path.join(data_sampled_path, 'motor3')
...
def main():
    ...
    run_uuid = "aa98bc5efbf111ee8daa7b9204547487"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
