...
from ffp2d_seedV_overlap_contrastive_pth2 import Configs
    ...
    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
        ...
        if overlap_cover:
            ...
            for i in range(x0.shape[3]):
                ...
                real_noises[:, :, 0 + i * self.step: self.window_size + i * self.step] = x0_noise[:, :, :, i]
                real_mix[:, :, 0 + i * self.step: self.window_size + i * self.step] = x_mix[:, :, :, i]
        data_sampled_path = '/media/administrator/F62C65672C652439/yexiyuan/contrast-experiments-progan-contrastive-_pth2'
...
def main():
    ...
    run_uuid = "c47b5d86e52911ee977d43c7939db293"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
