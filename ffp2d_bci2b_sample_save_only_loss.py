...
from src.EEGNET_2b import EEG_Net_8_Stack
...
from ffp2d_bci2b_overlap_only_loss import Configs
...
class ArcMarginHead(nn.Module):
    ...
    def __init__(self, in_features, out_features,
                 load_backbone='./assets/bci2b/max_acc.pth', s=30.0, m=0.50,
                 easy_margin=False):
...
class Sampler:
    ...
    def sample_save_data(self, n_frames: int = 1000, create_video: bool = False, overlap_cover=True, debug=False,
        ...
        for t_inv in monit.iterate('Denoise', self.n_steps):
            ...
            # =======================================
            # a = xt
            model = self.eps_model(3, self.out_channels, [32] * 4, [6] * 4)
        ...
        real_waves = np.ones((x0.shape[0], 3, 750))
        real_noises = np.ones((x0.shape[0], 3, 750))
        real_mix = np.ones((x0.shape[0], 3, 750))
        x_mix = x0 + x0_noise
        if overlap_cover:
            for i in range(x0.shape[3]):
                if debug:
            ...
            for i in range(x0.shape[3]):
                ...
                real_noises[:, :, 0 + i * self.step: self.window_size + i * self.step] = x0_noise[:, :, :, i]
                real_mix[:, :, 0 + i * self.step: self.window_size + i * self.step] = x_mix[:, :, :, i]
        data_sampled_path = '/media/administrator/F62C65672C652439/yexiyuan/bci_2b_progan_only_loss'
        output_path = os.path.join(data_sampled_path, 'motor2')
...
def main():
    ...
    run_uuid = "97f91b68f16811ee8daa7b9204547487"
    # Start an evaluation
    experiment.evaluate()
    # Create configs
    configs = Configs()
    ...
    with experiment.start():
        ...
        with torch.no_grad():
            ...
            for i in range(9):
                ...
                sampler.sample_save_data(n_frames=20, subject_id=i, batch_size=140)
